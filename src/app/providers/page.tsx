...
export default function ProvidersPage() {
  ...
  // Discovery Form State (Manual / Single URL)
  ...
  const handleAutoSearchSubmit = async (e: React.FormEvent) => {
    ...
    setAutoSearchStep('جارٍ استكشاف الويب والبحث عن أفضل مزودي التجارب...');

    const t1 = setTimeout(() => {
      setAutoSearchStep('تم العثور على روابط المواقع. جارٍ كشط الصفحات واستخراج النصوص...');
    }, 3500);

    const t2 = setTimeout(() => {
      setAutoSearchStep('جارٍ تحليل البيانات عبر نموذج الذكاء الاصطناعي (Groq Llama-3.1-70B)...');
    }, 7500);
    ...
    try {
      ...
      // Prepend newly discovered providers to list
  ...
  // Handle Discovery Submission (text, URL, or both)
  const handleDiscoverSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!rawText.trim() && !discoveryUrl.trim()) return;
    ...
    try {
      ...
      if (rawText.trim()) payload.text_content = rawText.trim();
      if (discoveryUrl.trim()) payload.url = discoveryUrl.trim();
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 pb-16">
      ...
      <div className="mx-auto max-w-screen-2xl px-6 pt-6 space-y-6">
        {/* Autonomous Web Search Section */}
        <div className="rounded-2xl border border-violet-200 bg-linear-to-r from-violet-950 via-indigo-950 to-slate-950 p-6 text-white shadow-md relative overflow-hidden">
          {/* Subtle background blur accents */}
          <div className="absolute -top-24 -left-24 h-64 w-64 rounded-full bg-violet-500/20 blur-3xl pointer-events-none" />
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-500/30 text-violet-300 ring-1 ring-violet-400/30">
                    <Bot className="h-4 w-4" />
                  </span>
                  <h2 className="text-base font-bold text-white tracking-wide">
                    بحث مستقل عبر الإنترنت (Autonomous Web Search)
                  </h2>
                ...
                <p className="mt-1 text-xs text-violet-200/80">
                  اكتب استعلام البحث باللغة الطبيعية؛ وسيقوم الوكيل بالبحث في الويب، كشط أفضل المواقع، استخراج بيانات المزودين وتوليد التضمين الشعاعي آلياً.
                </p>
              </div>

              {/* Suggestions */}
              <div className="hidden lg:flex items-center gap-1.5 flex-wrap">
                <span className="text-[11px] font-bold text-violet-300/60">اقتراحات:</span>
                {AUTO_SEARCH_SUGGESTIONS.map((sugg, idx) => (
                  <button
                    key={idx}
                    type="button"
                    onClick={() => setAutoSearchQuery(sugg)}
                    className="rounded-lg bg-white/10 hover:bg-white/20 border border-white/10 px-2.5 py-1 text-[11px] font-medium text-violet-100 transition-colors cursor-pointer"
                  >
                    {sugg}
                  </button>
                ))}
            ...
            <form onSubmit={handleAutoSearchSubmit} className="space-y-3">
              <div className="flex flex-col sm:flex-row gap-2.5">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="اكتب استعلام البحث (مثال: Find stargazing camps in AlUla أو رحلات غوص في جدة)..."
                    disabled={isAutoSearching}
                    className="w-full rounded-xl border border-white/20 bg-white/10 ps-10 pe-4 py-3 text-xs text-white placeholder:text-violet-300/60 focus:border-violet-300 focus:bg-white/15 focus:ring-1 focus:ring-violet-300 shadow-inner backdrop-blur-sm transition-all"
                  />
                </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-xl bg-violet-600 hover:bg-violet-500 active:bg-violet-700 px-6 py-3 text-xs font-bold text-white shadow-md transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                >
                  {isAutoSearching ? (
                    <>
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
                      <span>جارٍ الاستكشاف التلقائي...</span>
                    </>
                  ) : (
                    <>
              ...
              {/* Progress Indicator */}
              {isAutoSearching && (
                <div className="rounded-xl bg-white/10 border border-white/15 p-3.5 flex items-center gap-3 animate-pulse">
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-violet-300 border-t-transparent shrink-0" />
                  <div className="text-xs text-violet-100 font-medium flex-1">
                    {autoSearchStep || 'جارٍ تنفيذ دورة البحث والاستكشاف التلقائية عبر الويب (قد تستغرق 10-15 ثانية)...'}
                  </div>
                  <span className="text-[10px] text-violet-300/80 font-mono">10-15 ثانية</span>
                </div>
              )}

              {/* Error feedback */}
              {autoSearchError && (
                <div className="rounded-xl bg-red-950/80 border border-red-500/30 p-3 text-xs font-semibold text-red-200">
                  {autoSearchError}
                </div>
              )}
              ...
              {autoSearchResult && (
                <div className="rounded-xl bg-emerald-950/80 border border-emerald-500/40 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <CheckCircle2 className="h-4 w-4 text-emerald-400" />
                      <span className="text-xs font-bold text-emerald-200">
                        اكتملت دورة البحث المستقل! تم استخراج وحفظ {autoSearchResult.count} مزود تجارب في قاعدة البيانات.
                      </span>
                    </div>
                    <span className="rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-bold px-2.5 py-0.5 border border-emerald-500/30">
                      384 أبعاد (Local AI)
                  ...
                  {autoSearchResult.urls_scraped && autoSearchResult.urls_scraped.length > 0 && (
                    <div className="pt-1">
                      <span className="text-[10px] text-emerald-300/80 block mb-1">المواقع التي قام الوكيل بكشطها وتحليلها:</span>
                      <div className="flex flex-wrap gap-2">
                        {autoSearchResult.urls_scraped.map((u, i) => (
                          <a
                            ...
                            className="inline-flex items-center gap-1 rounded-md bg-white/10 px-2 py-0.5 text-[10px] text-emerald-200 hover:bg-white/20 font-mono transition-colors"
                            dir="ltr"
                          >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1 border-t border-emerald-500/20 text-xs">
                    {autoSearchResult.providers.map((p) => (
                      <div key={p.id} className="bg-white/5 rounded-lg p-2.5 border border-white/10">
                        <div className="font-bold text-white text-xs">{p.name}</div>
                        <div className="text-[11px] text-emerald-300/90 mt-0.5 flex items-center justify-between">
                          <span>{p.city} • {p.experience_type}</span>
                          {p.phone_number && <span className="font-mono text-[10px]" dir="ltr">{p.phone_number}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-gray-500">
                أدخل رابط صفحة مزود التجارب أو ألصق نصاً غير مهيكل (منشور إنستغرام، رسالة واتساب، نبذة تعريفية) وسيتولى الذكاء الاصطناعي استكشاف البيانات وتوليد التضمين الشعاعي آلياً.
              </p>
            </div>
          ...
          <form onSubmit={handleDiscoverSubmit} className="space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="https://example.com/provider-page — أدخل رابط صفحة مزود التجربة للاستكشاف التلقائي"
                dir="ltr"
                className="w-full rounded-xl border border-violet-200 bg-violet-50/30 ps-10 pe-3.5 py-2.5 text-xs leading-relaxed text-gray-900 placeholder:text-gray-400 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 shadow-xs font-mono"
              />
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold text-gray-400">أو ألصق نصاً يدوياً</span>
              <div className="flex-1 h-px bg-gray-200" />
            </div>
            ...
            <div className="flex justify-end gap-2">
              <button
                type="submit"
                disabled={isExtracting || (!rawText.trim() && !discoveryUrl.trim())}
              ...
              >
                {isExtracting ? (
                  <>
                    ...
                    <span>{discoveryUrl.trim() ? 'جارٍ الاستكشاف من الرابط...' : 'جارٍ الاستخراج بالذكاء الاصطناعي...'}</span>
                  </>
                ) : discoveryUrl.trim() ? (
                  <>
                    <Globe className="h-3.5 w-3.5" />
                    <span>اكتشاف من الرابط</span>
                  </>
                ) : (
                  <>
            ...
            {filteredProviders.map((provider) => (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400">
                  ...
                  <span className="font-mono text-gray-300">pgvector: 384d</span>
                </div>
              </div>
            ))}
          </div>
